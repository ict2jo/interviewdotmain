...
export default function ResultDetails({ children, title }) {
  return (
    <div className='w-[12rem] h-[12rem]'>
      <div className='mt-5 border-b-2 border-dashed font-bold flex flex-col items-center gap-3'>
        <span>{title && <IconComponent icon={title} />}</span>
        <span className='text-xs'>{title}</span></div>
      <p className='mt-3 whitespace-wrap text-xs text-center'> {children}</p>
    </div>
  )
}
